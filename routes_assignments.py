...
from flask import (
    ...
    url_for, session, flash, jsonify, abort, send_file, Response
)
import MySQLdb
...
# ═══════════════════════════════════════════════════════
#  SERVE SUBMISSION PDF  (teacher views student PDF inline)
# ═══════════════════════════════════════════════════════

@assignments_bp.route('/submission/<int:sub_id>/pdf')
def serve_submission_pdf(sub_id):
    """Serve a student's uploaded PDF file to the teacher for inline viewing."""
    if 'user_id' not in session:
        abort(401)

    cursor = _cursor()
    cursor.execute(
        '''SELECT s.file_path, s.filename, s.student_id, a.classroom_id
    ...
    # Access control: only the submitting student OR the classroom instructor can view
    if session.get('role') == 'instructor':
        cursor.execute('SELECT id FROM classrooms WHERE id=%s AND instructor_id=%s',
                       (row['classroom_id'], session['user_id']))
        if not cursor.fetchone():
            abort(403)
    elif session['user_id'] != row['student_id']:
        abort(403)
...
@assignments_bp.route(
    '/classroom/<int:classroom_id>/assignment/<int:assignment_id>/teacher_submit/<int:student_id>',
    methods=['POST']
)
def teacher_submit_for_student(classroom_id, assignment_id, student_id):
    """Teacher uploads a PDF and triggers AI grading for a specific student."""
    ...
    if not f or not f.filename or not _allowed(f.filename):
        return jsonify({'ok': False, 'error': 'Invalid file. Upload PDF, DOC, or DOCX.'}), 400

    save_dir = Path(Config.UPLOAD_FOLDER) / 'submissions' / str(classroom_id) / str(student_id)
    save_dir.mkdir(parents=True, exist_ok=True)
    fn = f'{uuid.uuid4().hex}_{secure_filename(f.filename)}'
    fpath = save_dir / fn
    f.save(str(fpath))

    # Upsert: if student already has a non-locked submission, replace it; else insert
    cursor.execute(
    ...
    if existing:
        if existing['locked']:
            return jsonify({'ok': False, 'error': 'Student submission is already locked.'}), 400
        sub_id = existing['id']
        cursor.execute(
            '''UPDATE assignment_submissions
               SET filename=%s, file_path=%s, submitted_at=NOW(),
                   ai_grade=NULL, ai_grade_label=NULL, ai_feedback=NULL, locked=0
               WHERE id=%s''',
            (fn, str(fpath), sub_id)
        )
    else:
        cursor.execute(
            '''INSERT INTO assignment_submissions
               ...
               VALUES (%s,%s,%s,%s,%s,0)''',
            (assignment_id, student_id, fn, str(fpath), '')
        )
    ...
    rubric = _sanitize(rubric_extracted + f"\nTitle: {assignment['title']}\nMax marks: {assignment['max_marks']}")
        ...
        try:
            ...
            try:
                ...
                c2.execute(
                    ...
                    (_sanitize(f'GRADING ERROR: {exc}')[:2000], sub_id)
                )
                db.commit()
            except Exception:
                pass
    ...
    return jsonify({'ok': True, 'sub_id': sub_id,
                    'message': 'File submitted. AI grading pipeline started.'})


# ═══════════════════════════════════════════════════════
#  TEACHER — MANUAL GRADE OVERRIDE  (JSON API)
# ═══════════════════════════════════════════════════════

@assignments_bp.route('/assignment/submission/<int:sub_id>/override', methods=['POST'])
def override_submission_grade(sub_id):
    """Teacher can override the AI grade and add custom feedback."""
    redir = _require_login('instructor')
    if redir: return redir
...
def create_assignment(classroom_id):
    ...
    if request.method == 'POST':
        ...
        if _has_col('rubric'):         cols.append('rubric');         vals.append(rubric)
        if _has_col('assign_text'):    cols.append('assign_text');    vals.append(assign_text)
        if _has_col('source_label'):   cols.append('source_label');   vals.append(source_label)
        if _has_col('ai_model'):       cols.append('ai_model');       vals.append(ai_model)
        if _has_col('strictness'):     cols.append('strictness');     vals.append(strictness)
        if _has_col('feedback_style'): cols.append('feedback_style'); vals.append(feedback_style)
        if _has_col('max_attempts'):   cols.append('max_attempts');   vals.append(max_attempts)
        if _has_col('visibility'):     cols.append('visibility');     vals.append(visibility)

        placeholders = ', '.join(['%s'] * len(cols))
        col_list     = ', '.join(f'`{c}`' for c in cols)
        cursor.execute(f"INSERT INTO assignments ({col_list}) VALUES ({placeholders})", vals)
        _commit()
...
def edit_assignment(classroom_id, assignment_id):
    ...
    if request.method == 'POST':
        ...
        if _has_col('rubric'):       set_parts.append('rubric=%s');       set_vals.append(rubric)
        if _has_col('assign_text'): set_parts.append('assign_text=%s'); set_vals.append(assign_text)
        if _has_col('max_attempts'):set_parts.append('max_attempts=%s'); set_vals.append(max_attempts)
        if _has_col('visibility'):  set_parts.append('visibility=%s');  set_vals.append(visibility)

        set_vals.append(assignment_id)
        cursor.execute(f"UPDATE assignments SET {', '.join(set_parts)} WHERE id=%s", set_vals)
        _commit()
...
def classroom_analytics(classroom_id):
    ...
    scores = [r['teacher_grade'] if r['teacher_grade'] is not None else r['ai_grade']
              for r in all_subs if r['teacher_grade'] is not None or r['ai_grade'] is not None]
    ...
    for r in raw_per:
        ...
        per_assignment.append({
            ...
            'completion_pct': round((r['sub_count'] / total_students) * 100, 1),
        })
...
def view_assignment(classroom_id, assignment_id):
    ...
    return render_template('assignments/student_view.html',
                           classroom=classroom, assignment=assignment,
                           existing_submission=existing, now=datetime.now())
...
def submit_assignment_v2(classroom_id, assignment_id):
    ...
    if 'RUBRIC:' in raw_desc.upper():
        ...
        rubric_extracted = assignment.get('rubric', '') or raw_desc
    rubric = _sanitize(f"{rubric_extracted}\nTitle: {assignment['title']}\nMax marks: {assignment['max_marks']}")
    ...
    def _grade_thread():
        ...
        try:
            ...
            if file_path_str:
                result   = evaluate_assignment(submission_pdf=file_path_str, rubric=rubric,
                                               course_id=course_id_str, student_id=student_id_str)
                score    = float(result.get('score', 0))
                feedback = _sanitize(str(result.get('feedback', '') or result.get('evaluation', '')))
            else:
                result   = evaluate_project(repo_url=github_url, project_rubric=rubric,
                                            project_details=assignment.get('description', ''),
                                            student_id=student_id_str, course_id=course_id_str)
                score    = float(result.get('score', 0))
            ...
            try:
                ...
                c2 = db.cursor()
                c2.execute("UPDATE assignment_submissions SET ai_feedback=%s WHERE id=%s",
                           (_sanitize(f'GRADING ERROR: {exc}')[:2000], sub_id))
                db.commit()
            except Exception:
                pass
    ...
    threading.Thread(target=_grade_thread, daemon=True).start()
    return jsonify({'ok': True, 'sub_id': sub_id,
                    'message': 'Submission received! AI is evaluating (30–90 seconds).'})
...
def submission_status(sub_id):
    ...
    graded = row['locked'] == 1
    error  = bool(row.get('ai_feedback') and str(row.get('ai_feedback', '')).startswith('GRADING ERROR'))
    return jsonify({'graded': graded, 'error': error,
                    'score': row['ai_grade'], 'label': row['ai_grade_label']})
...
def submission_result(sub_id):
    ...
    weaknesses  = _extract_section(feedback, 'WEAKNESSES',
                                   ['IMPROVEMENTSUGGESTIONS', 'IMPROVEMENT_SUGGESTIONS',
                                    'DETAILEDFEEDBACK', 'DETAILED_FEEDBACK'])
    detailed    = _extract_section(feedback, 'DETAILEDFEEDBACK', []) or \
                  _extract_section(feedback, 'DETAILED_FEEDBACK', [])

    return render_template('assignments/result.html',
                           sub=sub, feedback=feedback,
                           criteria=criteria, suggestions=suggestions,
                           strengths=strengths, weaknesses=weaknesses,
                           detailed=detailed, classroom_id=sub['classroom_id'])
...
def assignment_leaderboard(classroom_id, assignment_id):
    ...
    for rank, r in enumerate(rows, 1):
        ...
        display_name = r['full_name'] if (is_teacher or is_you) else f'Student #{rank:03d}'
        leaderboard.append({
            'rank':         rank,
    ...
    return render_template('assignments/leaderboard.html',
                           ...
                           classroom_id=classroom_id, is_teacher=is_teacher)
